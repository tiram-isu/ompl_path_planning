...
def create_camera(name, fov, near_clip, far_clip):
...
file_path = bpy.path.abspath("//PRM_door.json")  # Adjust the path as needed
...
if not os.path.exists(file_path):
    ...
    if paths:
        counter = 0
        for path in paths:
            name = "Camera_" + str(counter)
            camera = create_camera(name, fov, near_clip, far_clip)
            total_frames = animate_camera(camera, path, speed_factor=speed_factor)
